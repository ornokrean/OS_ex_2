...
void SleepingThreadsList::remove_thread(int tid)
{
    SleepingThreadsList sleep_stack;
    while (peek() != NULL && peek()->id != tid)
    {
        sleep_stack.add(peek()->id, peek()->awaken_tv);
        pop();
    }
    if (peek() != NULL && peek()->id == tid)
    {
        pop();
    }
    while (sleep_stack.peek() != NULL && sleep_stack.peek()->id != tid)
    {
        add(sleep_stack.peek()->id, sleep_stack.peek()->awaken_tv);
        sleep_stack.pop();
    }
